...
abstract class TSaslTransport extends TTransport {
     ...
     */
    public function __construct(\Thrift\Transport\TTransport $transport, \Examples\Auth\SASL\Client $client = null) {
     ...
     * @throws \Examples\Auth\SASL\Exception
     */
     ...
     * payload length in the header, then wrap the result in an instance of \Examples\Auth\SASL\Response
     * 
     * @return \Examples\Auth\SASL\Response
    ...
    protected function receiveSaslMessage() {
        ...
        $this->logger->debug(sprintf("%s: Received message with status %s and payload length %d",
                $this->getRole(), static::$statusMap[$status], $length));        
        return new \Examples\Auth\SASL\Response($status, $payload);
    }
     ...
     * @param \Logger $logger
     * @return \Examples\Thrift\Transport\TSaslTransport
     */
