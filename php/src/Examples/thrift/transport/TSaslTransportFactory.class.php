...
class TSaslTransportFactory {
    ...
    public static function factory(\Thrift\Transport\TTransport $transport, $mechanismName, $username, $password) {
        if($mechanismName == \Examples\Auth\SASL\Mechanism\BaseMechanism::NOSASL) {
            return $transport;
        }

        $sasl = new \Examples\Auth\SASL\ClientImpl();
        
        if(!is_null($mechanism = \Examples\Auth\SASL\Mechanism\Factory::factory($mechanismName, $sasl))) {
